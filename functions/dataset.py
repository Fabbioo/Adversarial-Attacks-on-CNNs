...
def load_dataset(images_path: str, added_new_images: bool = False) -> list:
    ...
    if added_new_images:
        ...
        del extension, file, files, num
    
    # Creo il dataset con tutte le immagini su cui eseguire gli attacchi.
    images: list = [os.path.join(images_path, elem) for elem in os.listdir(images_path) if not elem.startswith('.')]
    dataset: list = [image for image in images]
